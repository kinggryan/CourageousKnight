...
if(floor((mouse_y - start_y) / BLOCK_SIZE) != curr_offset)
{
    ...
    if(change_in_offset > 0)
        ...
        for(i = 0 ; i > change_in_offset ; i--)
        {
            while(temp_passage.next_up != first_passage_id)
            {
                ...
                if(temp_passage.next_down.next_left != temp_passage.next_down) // as long as it's not the only block in the column
                {
